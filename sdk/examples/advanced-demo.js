import ResShareClient, { ApiError } from '../src/index.js';
...
async function main() {
  ...
  const ownerClient = new ResShareClient({ baseUrl });
  const receiverClient = new ResShareClient({ baseUrl });
  ...
  const savedFile = path.join("sdk/examples/out", meetingNotesName);
  ...
  const zipPath = path.join("sdk/examples/out", `${rootFolder}.zip`);
  ...
  const receiverSavedPath = path.join("sdk/examples/out", `shared-from-${firstFromUser}-${path.basename(firstFilePath)}`);
  ...
  const receiverZipPath = path.join("sdk/examples/out", `shared-from-${firstFromUser}-${firstNode.name}.zip`);
